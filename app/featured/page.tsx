...
export default function FeaturedProductsPage() {
  ...
  const fetchFeatured = async (pageNumber: number, append = false) => {
    try {
      ...
      // Simulate pagination (local slicing)
      const paginated = data.slice(pageNumber * limit, (pageNumber + 1) * limit);

      setProducts((prev) => (append ? [...prev, ...paginated] : paginated));
      setError(null);
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (
        ...
        setPage(nextPage);
        fetchFeatured(nextPage, true);
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [isMobile, page, loading]);
